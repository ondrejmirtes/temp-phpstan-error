...
class Data_MySQL_DB{
	...
	const	MYSQL_ERROR__LOCK_WAIT_TIMEOUT		= 1205,
			...
			MYSQL_ERROR__TOO_LONG_STRING        = 1406
	;
	...
	public static function getInstance($connection_type=self::READ){
		$config = app_env();
		if($config['database']['unify_read_write']){
	...
	private function __construct(array $conf_data) {
		...
		$this->NativeDB = new PDO($dns,$conf_data['username'],$conf_data['password'],[PDO::MYSQL_ATTR_INIT_COMMAND => 'SET NAMES utf8mb4']);
	 ...
	 * @param array $params
	 * @throws PDOException
	 * @throws Data_MySQL_Exception_DuplicateEntry
	 * @throws Data_MySQL_Exception_TooLongString
	 * @return void
	 */
	private function execute($sql, array $params=[]){
		...
		try{
			...
			if($this->errorRecoveryCycles>10){
			    /* need to think what to do with this
			...
			switch($code){
				...
				case (self::MYSQL_ERROR__TOO_LONG_STRING)://String data, right truncated: 1406 Data too long for column <column name>
				    throw new Data_MySQL_Exception_TooLongString(print_r($this->lastBindParams,true));
