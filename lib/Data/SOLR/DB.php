...
class Data_SOLR_DB{
	...
	public function insert($data){
		...
		if(self::$last_status < 200 || self::$last_status >= 300){
		    error('Insert in SOLR faild ->'.$this->response->getContent());
		    dbgn(self::$last_status);
		}
		return $this;
	}
	...
	public function delete($id){
		...
		if(is_array($id)){
		    foreach ($id as $course_id){
		        $query.="<id>$course_id</id>";
		    }
		    $data = "<delete>{$query}</delete>";
		}else{
		  $query = 'id:' . $id;
