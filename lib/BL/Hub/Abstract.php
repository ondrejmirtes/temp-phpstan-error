...
abstract class BL_Hub_Abstract{
	...
	/**
	 * Determines if the class has a delta table to record changes defaults to false
	 * @var boolean
	 */
	protected $hasDelta = false;
	
	/**
	 * If true, will call the delta dbname_table_name
	 * @var boolean
	 */
	protected bool $delta_table_name_new_format = false;
   ...
   /**
	 ...
	 *
	 * @param ?int $id main entity id (like course, user, org)
	 * @param array $records
	 * @return integer last inserted array
	 */
	...
	/**
	 *  Create record and ignore duplicate record
	 *  
	 *  @param array $record
	 *  @return integer last inserted
	 */
	static public function createIgnoreRecord(array $record):int{
	    return self::getInstance(null)->insertData($record,false,true);
	}
	
	/**
	 *  Create records and ignore duplicate records
	 *  
	 *  @param array $records
	 *  @return boolean 
	 */
	static public function createIgnoreMultipleRecords(array $records):bool{
	    return self::getInstance(null)->insertMultipleData($records,false,true);
	}
	...
	protected function cleanData($data) {
	    // TODO see how this function works - looks strange TOBEDELETED???
	 ...
	 * @return Data_MySQL_DB
	 */
	public function insertData(array $data, $on_duplicate_update=false, $ignore = false){
		if($this->recordsIds && $on_duplicate_update){
			$data['id']=$this->recordsIds;
		}
		$data=[$data];
		return $this->insertMultipleData($data,$on_duplicate_update, $ignore);
	}
	 ...
	 * @return Data_MySQL_DB
	 */
	public function insertMultipleData(array $data,$on_duplicate_update=false, $ignore = false){
	...
	protected function insertMultipleDataRaw(array $data,$on_duplicate_update=false, $ignore = false){
		...
		$sql="INSERT {$ignore_sql}INTO {$this->databaseName}.{$this->tableName} (`{$fields_str}`,date_created,created_by,modified_by)\nVALUES\n";
		$params=array();
	 ...
	 * @return string
	 */
	protected function get_delta_table_name():string{
	    if($this->delta_table_name_new_format){
	...
	public function createHistoryRecord(array $where=array(),$type) {
		...
		$delta_table_name = $this->get_delta_table_name();
		
		$sql="
		INSERT INTO lms2delta.{$delta_table_name}
	...
	protected function sendEventMessage($command,$where){
		$msg = DataPusher_ActiveMQ_Publisher_HubEvents::construct_message($this->databaseName,$this->tableName,$command,$where);
		return DataPusher_ActiveMQ_Publisher_HubEvents::get_client(app_env()['database']['activeMQ'])->publish($msg);
	}
	 ...
	 *  @param array  $concat_params	Array of params that require concatination
	 */
