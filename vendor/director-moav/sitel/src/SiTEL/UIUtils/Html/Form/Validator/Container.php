<?php namespace SiTEL\UIUtils\Html\Form\Validator;
...
class Container{
    ...
    /**
     * adds to validators which is an [element name]->validators->validator_type = {config values}
     * 
     * @param \SiTEL\UIUtils\Html\Form\Element\aElement $element
     * @param string $message
     * @return Container
     */
    public function add_required(\SiTEL\UIUtils\Html\Form\Element\aElement $element,string $message=''):Container{
        $this->add($element);
     ...
     * @param \SiTEL\UIUtils\Html\Form\Element\aElement $element
     */
    private function add(\SiTEL\UIUtils\Html\Form\Element\aElement $element):void{
        if(!isset($this->validators[$element->name()])){
            $this->validators[$element->name()] = \SiTEL\array_to_object([
    ...
    private function header():\stdClass{
        ...
        $header->excluded      = [];
        $header->feedbackIcons = \SiTEL\array_to_object([
    ...
    public function get_json():string{
        ...
        $json = json_encode($full_js);
        if(!$json){
            \ZimLogger\MainZim::$CurrentLogger->error('No validators where found',true);
            return '{}';
        } else {
            return $json;
        }
