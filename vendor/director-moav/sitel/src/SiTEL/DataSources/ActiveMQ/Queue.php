...
abstract class Queue implements \Countable
{
     ...
     * @return Queue
     */
    static public function get_client(array $config):\SiTEL\DataSources\ActiveMQ\Queue{
        $options = ['driverOptions' => ['host' => $config['host'],
                                        'port' => $config['port']]
        ];
        
        $logger = $config['logger'] ?? \ZimLogger\MainZim::factory2('nananananana',\ZimLogger\Streams\Nan::class,\ZimLogger\Streams\aLogStream::VERBOSITY_LVL_FATAL,'',true);
    ...
    public function __construct(array $options,$logger=null){
        if(!$logger){
            $logger = \ZimLogger\MainZim::factory2('nananananana',\ZimLogger\Streams\Nan::class,\ZimLogger\Streams\aLogStream::VERBOSITY_LVL_FATAL,'');
        }
        $this->logger = $logger; 
